...
with tab1:
    ...
    for i, row in property_rooms.iterrows():
        with st.container(border=True):
            st.markdown(
            f"""
            <span style="font-size:16px;font-weight:700;margin-bottom:-20px;">{row['room']}号室</span>
            <span style="font-size:14px;font-weight:500;margin-bottom:-10px;">　　💰総賃料 {row['total_rent']}/月</span>
            """,
            unsafe_allow_html=True
            )
